...
from datamodel.search.Tianzeshou_datamodel import TianzeshouProjectionLink
...
@Deleter(TianzeshouProjectionLink)
@Getter(TianzeshouProjectionLink)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(TianzeshouProjectionLink)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(TianzeshouProjectionLink, l)
